...
class NewsService extends BaseService {
  ...
  Future<DashboardResponse> getUserDashboardData() async {
    ...
    dashboardResponse.breakingNews =
        await getNewsFuture(newsType: NewsTypeBreaking);
    dashboardResponse.story = await getNewsFuture(newsType: NewsTypeStory);
    dashboardResponse.recentNews =
        await getNewsFuture(newsType: NewsTypeRecent);
  ...
  Query buildCommonQuery({String? newsType = '', String? searchText}) {
    ...
    if (newsType == NewsTypeBreaking) {
      query = ref!
          .where(NewsKeys.newsType, isEqualTo: newsType)
          .orderBy(CommonKeys.updatedAt, descending: true);
    } else if (newsType == NewsTypeStory) {
      query = ref!.where(NewsKeys.newsType, isEqualTo: newsType).where(
          CommonKeys.createdAt,
      ...
      query = ref!
          .where(NewsKeys.caseSearch, arrayContains: searchText)
          .orderBy(CommonKeys.updatedAt, descending: true);
    }

    return query.where(NewsKeys.newsStatus,
        whereIn: appStore.isAdmin
            ? [NewsStatusPublished, NewsStatusUnpublished, NewsStatusDraft]
            : [NewsStatusPublished]);
  }

  Future<List<NewsData>> getNewsFuture(
      {String newsType = '', String? searchText, int limit = DocLimit}) async {
    Query query;
    ...
    return await query.get().then((x) {
      return x.docs
          .map((y) => NewsData.fromJson(y.data() as Map<String, dynamic>))
          .toList();
    });
  }
  ...
  Future<List<NewsData>> getMostViewedNewsFuture({int limit = DocLimit}) async {
    Query query =
        ref!.orderBy(NewsKeys.postViewCount, descending: true).limit(limit);
  ...
  Future<List<NewsData>> getBookmarkNewsFuture() async {
    if (bookmarkList.isNotEmpty) {
      Query query = ref!
          .where(CommonKeys.id, whereIn: bookmarkList)
          .orderBy(CommonKeys.updatedAt, descending: true);
  ...
  Future<NewsData> newsDetail(String? id) async {
    return await ref!.doc(id).get().then(
        (value) => NewsData.fromJson(value.data() as Map<String, dynamic>));
  }

  Future<List<NewsData>> relatedNewsFuture(
      DocumentReference? doc, String? newsId) async {
    Query query = ref!
        .where(NewsKeys.newsStatus,
            whereIn: appStore.isAdmin
                ? [NewsStatusPublished, NewsStatusUnpublished, NewsStatusDraft]
  ...
  Future<CategoryData> getNewsCategory(DocumentReference doc) async {
    return await doc.get().then(
        (value) => CategoryData.fromJson(value.data() as Map<String, dynamic>));
  }
